...
def apply(
    ...
    then --label-enable scope, then name include list, then name exclude
    list (exclude always wins last).
    """
    ...
    for container in containers:
        ...
        if not container.is_monitored():
            continue
        if settings.label_enable and container.labels.get(ENABLE_LABEL, "").lower() != "true":
            continue
        if settings.include_names and container.name not in settings.include_names:
            continue
